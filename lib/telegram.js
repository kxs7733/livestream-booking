...
async function sendRescheduleRejectionNotification(creatorApp, historyRow, reason) {
  const chatId = await getTelegramChatId(creatorApp.telegram);
  if (!chatId) return;
  const originalTime = historyRow.oldStreamTime && historyRow.oldStreamEndTime
    ? historyRow.oldStreamTime + ' – ' + historyRow.oldStreamEndTime
    : (historyRow.oldStreamTime || '');
  const contactString = await getInternalPicContactString();

  const message =
    '<b>Your reschedule request has been rejected.</b>\n\n' +
    '<b>Shop:</b> ' + (creatorApp.brandName || creatorApp.shopName || '') + '\n' +
    '<b>Your original slot still stands:</b>\n' + formatDateDDMMMYYYY(historyRow.oldStreamDate) + (originalTime ? ' ' + originalTime : '') + '\n\n' +
    '<b>Reason:</b>\n' + (reason || '') + '\n\n' +
    'Please adhere to the original stream date and time above.\n\n' +
    'Thank you.\n\n' +
