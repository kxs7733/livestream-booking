'use strict';

function parseSender(emailFrom) {
  const match = String(emailFrom || '').match(/^(.*?)\s*<(.+?)>$/);
  if (match) return { name: match[1].trim(), email: match[2].trim() };
  return { email: String(emailFrom || '').trim() };
}

async function sendEmail(to, subject, text, opts = {}) {
  console.log('[email] sendEmail called to:', to, '| has key:', !!process.env.BREVO_API_KEY, '| EMAIL_FROM:', process.env.EMAIL_FROM);
  if (!to || !process.env.BREVO_API_KEY) {
    console.log('[email] early return — to:', to, 'BREVO_API_KEY set:', !!process.env.BREVO_API_KEY);
    return;
  }
  const sender = parseSender(process.env.EMAIL_FROM);
  const payload = {
    sender,
    to: [{ email: to }],
    subject,
    textContent: text,
  };
  if (opts.cc) {
    payload.cc = String(opts.cc).split(',').map(e => ({ email: e.trim() })).filter(e => e.email);
  }
  if (opts.html) {
    payload.htmlContent = opts.html;
  }
  try {
    const res = await fetch('https://api.brevo.com/v3/smtp/email', {
      method: 'POST',
      headers: {
        'api-key': process.env.BREVO_API_KEY,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(payload),
    });
    const body = await res.text();
    if (!res.ok) {
      console.error('[email] Brevo error:', body);
    } else {
      console.log('[email] Brevo response:', body);
    }
  } catch (err) {
    console.error('[email] Failed to send to', to, ':', err.message);
  }
}

module.exports = { sendEmail };
