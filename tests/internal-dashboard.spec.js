...
test.describe('Internal dashboard — counts and approval', () => {
  ...
  test('Pending confirmations stat card counts individual timeslot rows', async ({ page }) => {
    // CA_ROW_APPROVED_1 and CA_ROW_APPROVED_2 both have status=approved → 2 pending confirmations
    await expect(page.getByText('Pending Creator Confirmation')).toBeVisible();
    // The stat value should be 2 (2 rows, not 1 group)
  ...
  test('Pending confirmations view shows one card per timeslot row', async ({ page }) => {
