 ...
 *    - CreatorApplications: id | creatorId | creatorName | brandApplicationId | brandName | shopName | streamDate | streamTime | affiliateUsername | phone | telegram | shippingAddress | willingToTravel | status | createdAt
...
function addSeller(data) {
  ...
  appendRowAsText(sheet, [data.id, data.name, data.createdAt]);
...
function updateSellerProfile(shopId, data) {
  ...
  // 1. Update Sellers row
  var sellersSheet = getSheet('Sellers');
  var updated = updateRowById(sellersSheet, shopId, { name: data.name });
  if (!updated) return { success: false, error: 'Seller not found' };

  // 2. Update all BrandApplications where brandId matches
  ...
  // 3. Update all CreatorApplications that reference this brand's applications
  ...
  // Get all brand application IDs for this seller
  var brandAppIds = [];
