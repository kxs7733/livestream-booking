...
function validateCreatorLogin(affiliateId, affiliateUsername, phone) {
  // Gate: affiliate must exist in Managed Affiliates
  const managedSheet = getSheet('Managed Affiliates');
  if (managedSheet) {
    const managedData = managedSheet.getDataRange().getValues();
    const affIdCol = managedData[0].indexOf('affiliate_id');
    const isManaged = affIdCol !== -1 && managedData.slice(1)
      .some(function(row) { return String(row[affIdCol]).toLowerCase() === String(affiliateId).trim().toLowerCase(); });
    if (!isManaged) {
      return { success: false, error: 'Login Unavailable. Please contact Shopee Livestream Talent Management PIC - +65 9456 8465 if you have any questions.' };
    }
  }
...
var FALLBACK_INTERNAL_PIC = 'Shopee Livestream Talent Management PIC';
var FALLBACK_INTERNAL_PIC_NUMBER = '+65 9456 8465';
